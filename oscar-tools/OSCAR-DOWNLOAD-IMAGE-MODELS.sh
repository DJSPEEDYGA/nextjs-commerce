...
resolve_root() {
  ...
  if [[ -n "${1:-}" ]]; then
    ...
    default_root="$OSCAR_HOME"
  elif [[ -f ./.oscar-one-folder.env ]]; then
    default_root="$(pwd)"
  fi

  printf "\nOscar Image Model Downloader\n"
  printf "This script downloads or copies image models into Oscar's shared model store.\n"

  if [[ -n "$default_root" ]]; then
    read -r -p "Oscar root folder [$default_root]: " OSCAR_ROOT
    OSCAR_ROOT=${OSCAR_ROOT:-$default_root}
  else
...
load_env() {
  local envfile="$OSCAR_ROOT/oscar-image-runtimes.env"
  if [[ -f "$envfile" ]]; then
    # shellcheck disable=SC1090
    source "$envfile"
  else
    export OSCAR_IMAGE_MODEL_ROOT="$OSCAR_ROOT/models"
  fi
  MODEL_ROOT="${OSCAR_IMAGE_MODEL_ROOT:-$OSCAR_ROOT/models}"
  mkdir -p "$MODEL_ROOT"
...
Oscar shared image model store:
...
main_menu() {
  while true; do
    ...
    printf "  4) Copy existing local file/folder into Oscar\n"
