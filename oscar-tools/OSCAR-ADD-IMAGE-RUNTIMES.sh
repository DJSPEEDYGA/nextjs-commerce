...
_OSCAR_COMMON="$_SCRIPT_DIR/oscar-storage-common.sh"
...
if [[ -n "${1:-}" ]]; then
  ROOT_DEFAULT="$1"
elif [[ -n "${OSCAR_HOME:-}" ]]; then
  ROOT_DEFAULT="$OSCAR_HOME"
elif [[ -f ./.oscar-one-folder.env ]]; then
  ROOT_DEFAULT="$(pwd)"
fi

printf "\nOscar Image Runtime Installer\n"
printf "This adds local image generation runtimes into one shared Oscar folder.\n"
...
if [[ -n "$ROOT_DEFAULT" ]]; then
  read -r -p "Oscar root folder [$ROOT_DEFAULT]: " OSCAR_ROOT
  OSCAR_ROOT=${OSCAR_ROOT:-$ROOT_DEFAULT}
else
  read -r -p "Oscar root folder: " OSCAR_ROOT
fi
[[ -n "$OSCAR_ROOT" ]] || die "No Oscar root folder provided."
mkdir -p "$OSCAR_ROOT"
OSCAR_ROOT="$(cd "$OSCAR_ROOT" && pwd)"

ENV_FILE="$OSCAR_ROOT/.oscar-one-folder.env"
RUNTIME_ROOT="$OSCAR_ROOT/image-runtimes"
MODEL_ROOT="$OSCAR_ROOT/models"
CACHE_ROOT="$OSCAR_ROOT/cache"
OUTPUT_ROOT="$OSCAR_ROOT/outputs"
LOG_ROOT="$OSCAR_ROOT/logs"
SCRIPT_ROOT="$OSCAR_ROOT"
...
FREE_GB=$(df -Pk "$OSCAR_ROOT" | awk 'NR==2 {print int($4/1024/1024)}')
...
cat > "$SCRIPT_ROOT/oscar-image-runtimes.env" <<ENVVARS
export OSCAR_HOME="$OSCAR_ROOT"
export OSCAR_IMAGE_RUNTIME_ROOT="$RUNTIME_ROOT"
export OSCAR_IMAGE_MODEL_ROOT="$MODEL_ROOT"
export OSCAR_IMAGE_OUTPUT_DIR="$OUTPUT_ROOT/images"
export OSCAR_IMAGE_LOG_DIR="$LOG_ROOT"
...
export OSCAR_COMFY_PORT="8188"
export OSCAR_A1111_PORT="7860"
export OSCAR_FORGE_PORT="7861"
export OSCAR_INVOKEAI_PORT="9090"
ENVVARS

# Preserve earlier env if present by appending image runtime variables only once.
if [[ -f "$ENV_FILE" ]]; then
  if ! grep -q 'oscar-image-runtimes.env' "$SCRIPT_ROOT/START-OSCAR-ONE-FOLDER.sh" 2>/dev/null; then
    :
  fi
else
  touch "$ENV_FILE"
...
prompt = os.environ.get("DIFFUSERS_PROMPT", "A cinematic portrait of Oscar, futuristic offline AI operator, detailed lighting")
outdir = os.environ.get("OSCAR_IMAGE_OUTPUT_DIR", os.path.join(os.getcwd(), "outputs"))
...
cat > "$SCRIPT_ROOT/START-OSCAR-IMAGE-RUNTIMES.sh" <<'EOFSTART'
#!/usr/bin/env bash
set -euo pipefail
...
if [[ -d "$COMFYUI_DIR" ]]; then
  start_bg comfyui "source '$OSCAR_IMAGE_RUNTIME_ROOT/venvs/comfyui/bin/activate' && cd '$COMFYUI_DIR' && python main.py --listen 0.0.0.0 --port '$OSCAR_COMFY_PORT'" "$OSCAR_IMAGE_LOG_DIR/comfyui.log"
fi
if [[ -d "$A1111_DIR" ]]; then
  start_bg a1111 "cd '$A1111_DIR' && ./webui.sh --listen --api --port '$OSCAR_A1111_PORT'" "$OSCAR_IMAGE_LOG_DIR/a1111.log"
fi
if [[ -d "$FORGE_DIR" ]]; then
  start_bg forge "cd '$FORGE_DIR' && ./webui.sh --listen --api --port '$OSCAR_FORGE_PORT'" "$OSCAR_IMAGE_LOG_DIR/forge.log"
fi
if [[ -d "$INVOKEAI_DIR'" ]]; then :; fi
if [[ -d "$INVOKEAI_DIR" ]]; then
  start_bg invokeai "source '$OSCAR_IMAGE_RUNTIME_ROOT/venvs/invokeai/bin/activate' && invokeai-web --host 0.0.0.0 --port '$OSCAR_INVOKEAI_PORT'" "$OSCAR_IMAGE_LOG_DIR/invokeai.log"
fi

echo
echo "Endpoints:"
echo "ComfyUI:    http://127.0.0.1:${OSCAR_COMFY_PORT}"
echo "A1111:      http://127.0.0.1:${OSCAR_A1111_PORT}"
echo "Forge:      http://127.0.0.1:${OSCAR_FORGE_PORT}"
echo "InvokeAI:   http://127.0.0.1:${OSCAR_INVOKEAI_PORT}"
EOFSTART
chmod +x "$SCRIPT_ROOT/START-OSCAR-IMAGE-RUNTIMES.sh"

cat > "$SCRIPT_ROOT/STOP-OSCAR-IMAGE-RUNTIMES.sh" <<'EOFSTOP'
#!/usr/bin/env bash
set -euo pipefail
ROOT_DIR="$(cd "$(dirname "$0")" && pwd)"
# shellcheck disable=SC1091
...
for name in comfyui a1111 forge invokeai; do
  pidfile="$OSCAR_IMAGE_LOG_DIR/${name}.pid"
  if [[ -f "$pidfile" ]]; then
...
EOFSTOP
chmod +x "$SCRIPT_ROOT/STOP-OSCAR-IMAGE-RUNTIMES.sh"

cat > "$SCRIPT_ROOT/TEST-OSCAR-IMAGE-RUNTIMES.sh" <<'EOFTEST'
#!/usr/bin/env bash
set -euo pipefail
ROOT_DIR="$(cd "$(dirname "$0")" && pwd)"
# shellcheck disable=SC1091
...
check_url "ComfyUI"  "http://127.0.0.1:${OSCAR_COMFY_PORT}"
check_url "A1111"    "http://127.0.0.1:${OSCAR_A1111_PORT}/sdapi/v1/sd-models"
check_url "Forge"    "http://127.0.0.1:${OSCAR_FORGE_PORT}/sdapi/v1/sd-models"
check_url "InvokeAI" "http://127.0.0.1:${OSCAR_INVOKEAI_PORT}"

echo "Model store: $OSCAR_IMAGE_MODEL_ROOT"
echo "Outputs:     $OSCAR_IMAGE_OUTPUT_DIR"
EOFTEST
chmod +x "$SCRIPT_ROOT/TEST-OSCAR-IMAGE-RUNTIMES.sh"

cat > "$SCRIPT_ROOT/OSCAR-IMAGE-RUNTIME-REGISTER.txt" <<EOFREG
Add these endpoints to Oscar's tool registry / crew map:
...
Main scripts
------------
- OSCAR-ADD-IMAGE-RUNTIMES.sh
- START-OSCAR-IMAGE-RUNTIMES.sh
- STOP-OSCAR-IMAGE-RUNTIMES.sh
- TEST-OSCAR-IMAGE-RUNTIMES.sh
- oscar-image-runtimes.env

Run
---
1) Install runtimes:
   bash ./OSCAR-ADD-IMAGE-RUNTIMES.sh
...
say "Done. Image runtimes are now wired into Oscar at: $OSCAR_ROOT"
say "Next steps:"
printf '  1) bash "%s/START-OSCAR-IMAGE-RUNTIMES.sh"\n' "$SCRIPT_ROOT"
printf '  2) bash "%s/TEST-OSCAR-IMAGE-RUNTIMES.sh"\n' "$SCRIPT_ROOT"
