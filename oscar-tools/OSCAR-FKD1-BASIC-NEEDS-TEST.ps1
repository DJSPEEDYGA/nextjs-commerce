# Oscar FKD1 basic-needs test for Windows PowerShell.
# Verifies FKD1 drive access, GOAT/Oscar files, local model store, running APIs,
# and whether Oscar returns/creates real graphics instead of text-only drawing.

# Load multi-drive storage config if present (from OSCAR-CHOOSE-STORAGE.ps1).
$_ScriptRoot = Split-Path -Parent $MyInvocation.MyCommand.Path
$_StorageConfig = Join-Path $_ScriptRoot ".oscar-storage-config.ps1"
...
$MinFreeGB = 20
if ($env:OSCAR_MIN_FREE_GB) { [void][int]::TryParse($env:OSCAR_MIN_FREE_GB, [ref]$MinFreeGB) }
...
function Detect-OllamaStore([string]$DriveRoot, [string]$ScriptRoot) {
    $candidates = @(
        $env:OLLAMA_MODELS,
        $env:OSCAR_MODEL_STORE,
        $DriveRoot,
        (Join-Path $DriveRoot "ollama_data"),
        (Join-Path $DriveRoot "OscarModels\ollama_data"),
...
function Verify-ActualImageOutput([string]$DriveRoot) {
    ...
    try {
        ...
        if (Test-GraphicFile $out) { Pass "Oscar image-render bridge created an actual graphic: $out"; return }
        if ($res.Content -match 'data:image|base64') { Pass "Oscar image-render bridge returned image data in response"; return }
    } catch {}
    ...
    foreach ($action in @("image_render", "draw_image", "generate_image", "render_image")) {
        ...
        try {
            ...
            if (Test-GraphicFile $out) { Pass "Oscar /api/tools action '$action' created an actual graphic: $out"; return }
            if ($res.Content -match 'data:image|base64') { Pass "Oscar /api/tools action '$action' returned image data in response"; return }
        } catch {}
    }
...
$OscarRoot = Find-FirstDir @($env:OSCAR_ROOT, $DriveRoot, $ScriptRoot, (Join-Path $DriveRoot "USB-Uncensored-LLM-main"), (Join-Path $DriveRoot "GOAT-Royalty-App"))
if ($OscarRoot -and (Test-Path (Join-Path $OscarRoot "Shared\chat_server.py"))) { Pass "Oscar root found: $OscarRoot" } else { Fail "Oscar root with Shared\chat_server.py not found on FKD1" }
$Shared = if ($OscarRoot) { Join-Path $OscarRoot "Shared" } else { Join-Path $DriveRoot "Shared" }
...
$env:OSCAR_ROOT = $OscarRoot
$env:OSCAR_GOAT_APP_ROOT = $GoatRoot
$env:OSCAR_BRIDGE_WORKSPACE = $DriveRoot
$env:OSCAR_TOOL_WORKSPACE = $DriveRoot
$env:OSCAR_ASSET_ROOT = $DriveRoot
$env:OSCAR_OFFLINE = "1"
$env:GOAT_OFFLINE = "1"
$env:HF_HUB_OFFLINE = "1"
$env:TRANSFORMERS_OFFLINE = "1"
$env:OLLAMA_MODELS = Detect-OllamaStore $DriveRoot $ScriptRoot
$env:OSCAR_MODEL_STORE = $env:OLLAMA_MODELS
...
Pass "FKD1 environment configured for Oscar/GOAT"
...
Http-Check "Oscar tool policy" "http://127.0.0.1:3333/api/tools" '"actions"'
Http-Check "Oscar workspace bridge" "http://127.0.0.1:3333/api/workspace" '"root"'
