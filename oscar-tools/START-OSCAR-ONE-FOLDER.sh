...
mkdir -p "$OSCAR_LOG_DIR" "$OSCAR_HOME/runtime/pids" "$OSCAR_OUTPUT_DIR" "$OSCAR_IMAGE_OUTPUT_DIR"
...
start_ollama() {
  ...
  HOME="$OLLAMA_HOME" "$bin" serve > "$OSCAR_LOG_DIR/ollama-$STAMP.log" 2>&1 &
  echo $! > "$OSCAR_HOME/runtime/pids/ollama.pid"
  if wait_url "http://${OLLAMA_HOST}/api/tags" 45; then ok "Ollama started"; else warn "Ollama did not answer yet. Log: $OSCAR_LOG_DIR/ollama-$STAMP.log"; fi
}

start_drawing_bridge() {
  local port="${OSCAR_DRAW_PORT:-3344}"
  if command -v curl >/dev/null 2>&1 && curl -fsS --max-time 3 "http://127.0.0.1:${port}/health" >/dev/null 2>&1; then
    ok "Drawing bridge already running: $OSCAR_IMAGE_RENDER_ENDPOINT"
    return 0
  fi
  local py bridge
  py="$(find_python || true)"
  ...
  for bridge in \
    "$ROOT/OSCAR-FKD1-LOCAL-DRAWING-BRIDGE.py" \
    "$ROOT/OSCAR-LOCAL-DRAWING-BRIDGE.py" \
    "$(find_first_file 'OSCAR-FKD1-LOCAL-DRAWING-BRIDGE.py' || true)"; do
    if [ -n "$bridge" ] && [ -f "$bridge" ]; then
      info "Starting local drawing bridge: $OSCAR_IMAGE_RENDER_ENDPOINT"
      HOME="$OSCAR_RUNTIME_HOME" "$py" "$bridge" --root "$OSCAR_HOME" --host "${OSCAR_DRAW_HOST:-127.0.0.1}" --port "$port" > "$OSCAR_LOG_DIR/oscar-drawing-bridge-$STAMP.log" 2>&1 &
      echo $! > "$OSCAR_HOME/runtime/pids/drawing-bridge.pid"
      if wait_url "http://127.0.0.1:${port}/health" 15; then ok "Drawing bridge started"; else warn "Drawing bridge did not answer yet. Log: $OSCAR_LOG_DIR/oscar-drawing-bridge-$STAMP.log"; fi
      return 0
    fi
  done
  warn "Drawing bridge script not found. Oscar may describe images unless another image backend is configured."
}

start_oscar_bridge() {
  if command -v curl >/dev/null 2>&1 && curl -fsS --max-time 3 "http://127.0.0.1:${OSCAR_CHAT_PORT:-3333}/api/tools" >/dev/null 2>&1; then
    ok "Oscar tool/chat bridge already running on port ${OSCAR_CHAT_PORT:-3333}"
    return 0
  fi
  local py server
  py="$(find_python || true)"
  [ -n "$py" ] || { warn "Python not found; cannot start Oscar bridge."; return 0; }
  for server in \
    ...
    if [ -n "$server" ] && [ -f "$server" ]; then
      info "Starting Oscar bridge from: $server"
      HOME="$OSCAR_RUNTIME_HOME" "$py" "$server" --no-browser > "$OSCAR_LOG_DIR/oscar-chat-server-$STAMP.log" 2>&1 &
      echo $! > "$OSCAR_HOME/runtime/pids/oscar-chat-server.pid"
      if wait_url "http://127.0.0.1:${OSCAR_CHAT_PORT:-3333}/api/tools" 45; then ok "Oscar bridge started"; else warn "Oscar bridge did not answer yet. Log: $OSCAR_LOG_DIR/oscar-chat-server-$STAMP.log"; fi
      return 0
    fi
  done
  warn "Oscar chat_server.py not found inside this one-folder deployment."
}
...
start_goat_intel() {
  ...
  for intel in \
    ...
    if [ -n "$intel" ] && [ -f "$intel" ]; then
      ...
      (cd "$(dirname "$intel")" && HOME="$OSCAR_RUNTIME_HOME" "$py" "$intel") > "$OSCAR_LOG_DIR/goat-intel-$STAMP.log" 2>&1 &
      echo $! > "$OSCAR_HOME/runtime/pids/goat-intel.pid"
      sleep 2
      ok "GOAT Intel start attempted; log: $OSCAR_LOG_DIR/goat-intel-$STAMP.log"
      return 0
    fi
  done
}
...
start_goat_web() {
  ...
  HOME="$OSCAR_RUNTIME_HOME" "$py" -m http.server "${GOAT_WEB_PORT:-8765}" --bind 127.0.0.1 --directory "$web" > "$OSCAR_LOG_DIR/goat-web-$STAMP.log" 2>&1 &
  echo $! > "$OSCAR_HOME/runtime/pids/goat-web.pid"
  if wait_url "http://127.0.0.1:${GOAT_WEB_PORT:-8765}/" 20; then ok "GOAT web app started"; else warn "GOAT web app did not answer yet. Log: $OSCAR_LOG_DIR/goat-web-$STAMP.log"; fi
}

STAMP="$(date +%Y%m%d-%H%M%S)"
printf '\nOSCAR ONE-FOLDER START\nRoot: %s\n\n' "$OSCAR_HOME"
start_ollama
start_drawing_bridge
start_oscar_bridge
start_goat_intel
...
printf '\nLOCAL LINKS\n'
printf '  Oscar Chat/Tools:     http://127.0.0.1:%s/\n' "${OSCAR_CHAT_PORT:-3333}"
printf '  GOAT Web App:         http://127.0.0.1:%s/\n' "${GOAT_WEB_PORT:-8765}"
printf '  Drawing API:          %s\n' "$OSCAR_IMAGE_RENDER_ENDPOINT"
printf '  Ollama API:           http://%s/\n' "$OLLAMA_HOST"
printf '\nRun this to test everything:\n  bash %q/TEST-OSCAR-ONE-FOLDER.sh\n' "$OSCAR_HOME"
printf '\nRun this to stop services started by this launcher:\n  bash %q/STOP-OSCAR-ONE-FOLDER.sh\n' "$OSCAR_HOME"
