#!/usr/bin/env python3
"""
Oscar FKD1 Local Drawing Bridge
-------------------------------
Small offline graphics endpoint with zero third-party dependencies.
It is not a diffusion model; it is a guaranteed local graphics fallback so Oscar
...
def safe_root() -> Path:
    root = (os.environ.get("OSCAR_IMAGE_OUTPUT_DIR")
            or os.environ.get("OSCAR_DRIVE_OUTPUTS")
...
def render_svg(path: Path, prompt: str, width: int = 512, height: int = 512) -> None:
    ...
    text = (prompt or "Oscar graphic")[:90].replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
...
class Handler(BaseHTTPRequestHandler):
    server_version = "OscarFKD1DrawingBridge/1.0"
    ...
    def do_GET(self) -> None:  # noqa: N802
        if self.path in ("/", "/health", "/api/health"):
            self._send_json(200, {"ok": True, "service": "Oscar FKD1 Local Drawing Bridge", "root": str(self.server.root)})
            return
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            ...
            return
        prompt = str(payload.get("prompt") or payload.get("description") or "Oscar local graphic")
...
def main() -> int:
    ...
    parser.add_argument("--host", default=os.environ.get("OSCAR_DRAW_HOST", "127.0.0.1"))
    parser.add_argument("--port", type=int, default=int(os.environ.get("OSCAR_DRAW_PORT", "3344")))
    ...
    httpd.root = root
    print(f"Oscar FKD1 Local Drawing Bridge listening on http://{args.host}:{args.port}/api/draw")
