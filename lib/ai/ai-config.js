 ...
 * Integration with 215 NVIDIA LLM Tools, Google AI Studio, Lightning AI,
 * Hugging Face Inference Providers, and Open Source Model Registry
 * Updated: v3.2.0 - HuggingFace Model Hub + 16 Inference Providers + 40+ Models
 */

// Auto-detect demo mode when no API keys are configured
const isDemoMode = !process.env.NVIDIA_API_KEY && !process.env.LIGHTNING_API_KEY && !process.env.HF_TOKEN;

if (isDemoMode) {
    console.log('🎮 Running in DEMO MODE - AI features use simulated responses');
    console.log('   Set NVIDIA_API_KEY, LIGHTNING_API_KEY, or HF_TOKEN in .env to enable live AI');
}
...
module.exports = {
    ...
    // ==========================================
    // HUGGING FACE INFERENCE PROVIDERS (16 Providers)
    // ==========================================
    huggingface: {
        token: process.env.HF_TOKEN,
        providers: {
            groq: { apiKey: process.env.GROQ_API_KEY, speed: 'ultra-fast', specialty: 'Low-latency inference' },
            cerebras: { apiKey: process.env.CEREBRAS_API_KEY, speed: 'ultra-fast', specialty: 'Wafer-scale inference' },
            sambanova: { apiKey: process.env.SAMBANOVA_API_KEY, speed: 'fast', specialty: 'Enterprise AI' },
            together: { apiKey: process.env.TOGETHER_API_KEY, speed: 'fast', specialty: 'Open-source model hosting' },
            fireworks: { apiKey: process.env.FIREWORKS_API_KEY, speed: 'fast', specialty: 'Optimized inference' },
            novita: { apiKey: process.env.NOVITA_API_KEY, speed: 'standard', specialty: 'Multi-modal inference' },
            replicate: { apiKey: process.env.REPLICATE_API_KEY, speed: 'standard', specialty: 'Model marketplace' },
            cohere: { apiKey: process.env.COHERE_API_KEY, speed: 'fast', specialty: 'RAG & enterprise NLP' },
            scaleway: { apiKey: process.env.SCALEWAY_API_KEY, speed: 'standard', specialty: 'European cloud AI' },
            hyperbolic: { apiKey: process.env.HYPERBOLIC_API_KEY, speed: 'fast', specialty: 'Affordable GPU inference' },
            fal: { apiKey: process.env.FAL_API_KEY, speed: 'fast', specialty: 'Image & video generation' },
            featherless: { apiKey: process.env.FEATHERLESS_API_KEY, speed: 'fast', specialty: 'Serverless inference' },
            nscale: { apiKey: process.env.NSCALE_API_KEY, speed: 'standard', specialty: 'Scalable inference' },
            'hf-inference': { apiKey: process.env.HF_TOKEN, speed: 'standard', specialty: 'Official HF Inference API' }
        },
        localRunners: {
            ollama: {
                baseUrl: process.env.OLLAMA_URL || 'http://localhost:11434',
                enabled: !!process.env.OLLAMA_URL || false
            },
            vllm: {
                baseUrl: process.env.VLLM_URL || 'http://localhost:8000',
                enabled: !!process.env.VLLM_URL || false
            }
        }
    },
    ...
    modelRegistry: {
        ...
        defaultCategory: 'text-generation',
