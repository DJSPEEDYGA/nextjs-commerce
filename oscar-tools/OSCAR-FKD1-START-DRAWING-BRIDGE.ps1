...
$Bridge = Join-Path $ScriptRoot "OSCAR-FKD1-LOCAL-DRAWING-BRIDGE.py"
...
$env:OSCAR_IMAGE_RENDER_ENDPOINT = "http://127.0.0.1:3344/api/draw"
$env:GOAT_IMAGE_RENDER_ENDPOINT = $env:OSCAR_IMAGE_RENDER_ENDPOINT
$env:OSCAR_IMAGE_OUTPUT_DIR = $DriveRoot
try {
    Invoke-WebRequest -UseBasicParsing -Uri "http://127.0.0.1:3344/health" -TimeoutSec 3 | Out-Null
    Write-Host "Oscar FKD1 Local Drawing Bridge is already running at $($env:OSCAR_IMAGE_RENDER_ENDPOINT)" -ForegroundColor Green
    exit 0
} catch {}
...
try {
    ...
    Write-Host "Oscar FKD1 Local Drawing Bridge started: $($env:OSCAR_IMAGE_RENDER_ENDPOINT)" -ForegroundColor Green
