#!/usr/bin/env bash
# Oscar as-standing deploy/verification record — FKD1 local/offline routed edition.
# Oscar FKD1 basic-needs test.
# Verifies the drive, local app files, Oscar tools, crew profile endpoints,
...
MIN_FREE_GB="${OSCAR_MIN_FREE_GB:-20}"
...
start_oscar_if_needed() {
  if curl -fsS --max-time 5 http://127.0.0.1:3333/api/tools >/dev/null 2>&1; then
    pass "Oscar chat/tool bridge already listening on 127.0.0.1:3333"
    return
  fi
  if [ ! -f "$SHARED/chat_server.py" ]; then
    fail "Missing Oscar server: $SHARED/chat_server.py"
    return
  fi
  [ -n "$PYTHON_BIN" ] || { fail "Python 3 not found for Oscar server"; return; }
  info "Starting Oscar chat/tool bridge"
  "$PYTHON_BIN" "$SHARED/chat_server.py" --no-browser > "$LOG_DIR/oscar-chat-server-basic-needs-$STAMP.log" 2>&1 &
  if wait_url http://127.0.0.1:3333/api/tools 60; then pass "Oscar chat/tool bridge started"; else fail "Oscar chat/tool bridge did not start. Check $LOG_DIR/oscar-chat-server-basic-needs-$STAMP.log"; fi
}
...
verify_file_bridge_write_read() {
  ...
  payload="$DRIVE_ROOT/.oscar-tools-write-payload-$$.json"
  resp="$DRIVE_ROOT/.oscar-tools-write-response-$$.json"
  if [ -z "$PYTHON_BIN" ]; then warn "Skipping Oscar tool write test because Python is missing"; return; fi
  "$PYTHON_BIN" - "$payload" "$test_rel" <<'PYJSON'
import json, sys
payload, rel = sys.argv[1:3]
with open(payload, 'w', encoding='utf-8') as f:
    json.dump({"action":"write","path":rel,"content":"Oscar FKD1 basic needs write test\n"}, f)
PYJSON
  ...
  if [ "$code" = "200" ] && [ -f "$test_abs" ]; then
    pass "Oscar tool bridge can write into FKD1 workspace"
  elif [ "$code" = "200" ]; then
    warn "Oscar tool bridge replied HTTP 200 but write file was not found at $test_abs"
  else
    fail "Oscar tool bridge write test failed: HTTP ${code:-ERR} $(cat /tmp/oscar-write-curl-$$.err 2>/dev/null || true)"
  fi
...
PYJSON
  ...
  if is_graphic_file "$png_out"; then
    pass "Oscar image-render bridge created an actual graphic: $png_out"
    rm -f "$payload" "$resp" /tmp/oscar-image-curl-$$.err
    return
  fi
  if [ "$code" = "200" ] && grep -Eiq 'data:image|base64' "$resp" 2>/dev/null; then
    pass "Oscar image-render bridge returned image data in the response"
    rm -f "$payload" "$resp" /tmp/oscar-image-curl-$$.err
    return
  fi
...
PYJSON
    ...
    if is_graphic_file "$png_out"; then
      pass "Oscar /api/tools action '$action' created an actual graphic: $png_out"
      rm -f "$payload" "$resp" /tmp/oscar-image-curl-$$.err
      return
    fi
    if [ "$code" = "200" ] && grep -Eiq 'data:image|base64' "$resp" 2>/dev/null; then
      pass "Oscar /api/tools action '$action' returned image data in the response"
      rm -f "$payload" "$resp" /tmp/oscar-image-curl-$$.err
      return
    fi
  done

  fail "Oscar did not create or return a real graphic file. This confirms the description-only drawing issue still needs fixing in Oscar's image tool/back-end."
  ...
  if make_local_png_smoke "$fallback" "$PYTHON_BIN" && is_graphic_file "$fallback"; then
    warn "Local graphics fallback can create PNG files, so the problem is likely Oscar tool routing/model backend, not disk write access. Fallback proof: $fallback"
  else
...
if [ -n "$OSCAR_ROOT" ]; then pass "Oscar root found: $OSCAR_ROOT"; else fail "Oscar root with Shared/chat_server.py not found on FKD1"; fi
...
export OSCAR_MODEL_STORE="$OLLAMA_MODELS"
export OLLAMA_HOME OLLAMA_RUNNERS_DIR OLLAMA_TMPDIR OLLAMA_HOST

cat > "$DRIVE_ROOT/.oscar-fkd1.env" <<ENVFILE
# FKD1 Oscar/GOAT local environment generated by basic-needs test
export FKD1_ROOT="$DRIVE_ROOT"
export GOAT_DATA_ROOT="$DRIVE_ROOT"
...
[ -f "$WEB_APP/developers.html" ] && http_check "Developers Console via Oscar bridge" "http://127.0.0.1:3333/goat/developers.html" || warn "developers.html not found"

printf '\n============================================================\n'
printf 'OSCAR TOOL AND CREW API CHECKS\n'
printf '============================================================\n'
json_key_check "Oscar tool policy" "http://127.0.0.1:3333/api/tools" '"actions"'
json_key_check "Oscar workspace bridge" "http://127.0.0.1:3333/api/workspace" '"root"'
...
printf '\n'
printf 'Local links:\n'
printf '  Oscar Chat UI:       http://127.0.0.1:3333/\n'
