...
REPORT="$OSCAR_LOG_DIR/oscar-one-folder-test-$STAMP.txt"
mkdir -p "$OSCAR_LOG_DIR" "$OSCAR_OUTPUT_DIR" "$OSCAR_IMAGE_OUTPUT_DIR"
exec > >(tee "$REPORT") 2>&1
...
printf '============================================================\n'
printf 'OSCAR ONE-FOLDER BASIC NEEDS TEST\n'
printf '============================================================\n'
printf 'Timestamp: %s\n' "$(date)"
printf 'Root:      %s\n' "$OSCAR_HOME"
printf 'Report:    %s\n' "$REPORT"
...
[ -d "$OSCAR_HOME" ] && pass "One-folder root exists" || fail "One-folder root missing: $OSCAR_HOME"
touch "$OSCAR_HOME/.oscar-write-test" 2>/dev/null && { rm -f "$OSCAR_HOME/.oscar-write-test"; pass "One-folder root is writable"; } || fail "One-folder root is not writable"

for d in "$GOAT_DATA_ROOT" "$GOAT_DOWNLOAD_DIR" "$OSCAR_MODEL_STORE" "$OSCAR_GGUF_STORE" "$OSCAR_LOG_DIR" "$OSCAR_RUNTIME_DIR" "$OSCAR_IMAGE_OUTPUT_DIR"; do
  [ -d "$d" ] && pass "Folder exists: $d" || fail "Folder missing: $d"
done

for var in OSCAR_HOME FKD1_ROOT OSCAR_BRIDGE_WORKSPACE OSCAR_TOOL_WORKSPACE GOAT_DATA_ROOT GOAT_DOWNLOAD_DIR OSCAR_MODEL_STORE OLLAMA_MODELS OSCAR_GGUF_STORE HF_HOME TMPDIR; do
  val="${!var:-}"
  if [ -z "$val" ]; then
    fail "$var is empty"
  else
    case "$val" in
      "$OSCAR_HOME"*) pass "$var routes inside one folder -> $val" ;;
      *) fail "$var routes outside one folder -> $val" ;;
    esac
  fi
done

free_kb="$(df -Pk "$OSCAR_HOME" | awk 'NR==2 {print $4}')"
free_gb="$(awk -v kb="$free_kb" 'BEGIN { printf "%.1f", kb/1024/1024 }')"
min_gb="${OSCAR_MIN_FREE_GB:-50}"
...
[ -n "$chat" ] && pass "Oscar chat/tool bridge source found: $chat" || warn "Oscar chat_server.py not found"

draw="$(find_file OSCAR-FKD1-LOCAL-DRAWING-BRIDGE.py || true)"
...
if [ -x "$ROOT/START-OSCAR-ONE-FOLDER.sh" ]; then
  printf '\n[STEP] Starting local services for test...\n'
  bash "$ROOT/START-OSCAR-ONE-FOLDER.sh" || warn "Start script returned a warning/failure"
else
  warn "Start script missing"
fi

if command -v curl >/dev/null 2>&1; then
  wait_url "http://127.0.0.1:${OSCAR_DRAW_PORT:-3344}/health" 10 && pass "Drawing bridge health endpoint is live" || warn "Drawing bridge health endpoint is not live"
  test_img="$OSCAR_IMAGE_OUTPUT_DIR/oscar-basic-needs-drawing-$STAMP.png"
  if curl -fsS --max-time 20 -H 'Content-Type: application/json' \
    -d "{\"prompt\":\"Oscar basic needs test: make a real image file\",\"outputPath\":\"$test_img\",\"format\":\"png\",\"width\":256,\"height\":256}" \
    "$OSCAR_IMAGE_RENDER_ENDPOINT" >/tmp/oscar-draw-test-$$.json 2>/tmp/oscar-draw-test-$$.err; then
    if [ -s "$test_img" ]; then
      magic="$(head -c 8 "$test_img" | od -An -tx1 | tr -d ' \n')"
      if [ "$magic" = "89504e470d0a1a0a" ]; then
        pass "Oscar drawing bridge created a real PNG: $test_img"
      else
  ...
  wait_url "http://127.0.0.1:${OSCAR_CHAT_PORT:-3333}/api/tools" 5 && pass "Oscar tool API is live" || warn "Oscar tool API is not live"
...
if [ "$FAIL" -eq 0 ]; then
  printf 'OSCAR ONE-FOLDER STATUS: PASS / CHECK WARNINGS IF ANY\n'
  exit 0
fi
printf 'OSCAR ONE-FOLDER STATUS: CHECK FAILURES ABOVE\n'
exit 1
